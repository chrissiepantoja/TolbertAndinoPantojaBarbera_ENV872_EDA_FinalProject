...
https://github.com/chrissiepantoja/TolbertAndinoPantojaBarbera_ENV872_EDA_FinalProject
...
*Question 2: How these counties experience rainfall and flooding impacts differently?*
...
USGS stream gage data shows the latitude and longitude of the stream gage, discharge in cubic feet per second, and the date-time that the data was recorded. (This was already mostly in the paragraph above- we could keep this here or not)
...
For the selected sites we retrieve data from USGS using USGS's "dataRetrieval"
package. We want to compare as many parameters as possible between our sites, so
we selected all common parameters:
for stream discharge (00060) , precipitation (00045), and gage height (0065).
The relevant codes we will use are daily mean (0003) for Discharge and Gage
Height and sum (00006) for Precipitaion.

To clean the data we remove the 'cd' columns that indicate if the data is
official or provisional as this won't have a significant effect on our findings. 

We then clearly label each column with its parameter and stat code.

After initial retreival, I create a Water Data retrieval function for my chosen
parameters based on my previous retreival and use it for all data wrangling
going forward.
...
SugarGroveData <- invisible(suppressWarnings(suppressMessages(
  readNWISdata(  # dataRetrieval function from USGS API
...
```{r Discharge event over 2024, warning=FALSE, echo=FALSE, fig.cap = "Total Daily Precipitation Trends (Inches) for 2024"}
...
combined_plot <- plot_sg / plot_ash / plot_pg + 
  plot_annotation(
    ...
    subtitle = "Comparison across Sugar Grove, Asheville, and Pigeon River",
    theme = theme(
...
```{r Helene Discharge event over 2024, echo=FALSE, warning=FALSE, fig.cap = "Daily Discharge Trends (cfs) for 2024"}
...
### Daily Gage Height Trend

```{r Gage Height event over 2024, echo=FALSE, warning=FALSE, fig.cap = "Daily Gage Height Trends (cfs) for 2024"}
...
### Historical Precipitation Trend
...
combined_precipitation_plot <- (SugarGroveDailyPrecipitationPlot + 
  ...
  plot_annotation(
    title = "Historical Total Daily Precipitation (Inches) (1954-2024)",
    subtitle = "Comparison across Sugar Grove, Asheville, and Pigeon River",
    theme = theme(
...
combined_discharge_plot <- (SugarGroveDailyDischargeLinePlot + 
  ...
  plot_annotation(
    title = "Historical Mean Daily Discharge (cfs) (1954-2024)",
    subtitle = "Comparison across Sugar Grove, Asheville, and Pigeon River",
    theme = theme(
      plot.title = element_text(size = 20, face = "bold"),
      plot.subtitle = element_text(size = 16)
...
combined_gage_height_plot <- (SugarGroveDailyGageHeightPlot + 
  ...
  plot_annotation(
    title = "Historical Mean Daily Gage Height (ft) (1954-2024)",
    subtitle = "Comparison across Sugar Grove, Asheville, and Pigeon River",
    theme = theme(
      plot.title = element_text(size = 20, face = "bold"),
      plot.subtitle = element_text(size = 16)
...
### Sugar Grove Analysis: Decomposition Plot

```{r SUGAR GROVE, echo=FALSE}
...
### Asheville Analysis: Decomposition Plot

```{r ASHEVILLE, echo=FALSE}
...
### Pigeon River Analysis: Decomposition Plot

```{r PIGEON RIVER, echo=FALSE}
...
```{r Sugar Grove Precip Hazen, warning=FALSE, echo = FALSE}
...
knitr::kable(SugarGrovePrecipHazen2024, 
             ...
             digits = 2,
             label = "SugarGrovePrecipHazen",  # Add label to the table,
             align = "c",
...
knitr::kable(AshevillePrecipHazen2024, 
             ...
             digits = 2,
             label = "AshevillePrecipHazen",  # Add label to the table
             align = "c",
             col.names = c("Year", "Max Event", "Rank", "Fa Value", "Hazen Value"))  # Custom column names
...
knitr::kable(PigeonRiverPrecipHazen2024, 
             ...
             digits = 2,
             label = "PigeonRiverPrecipHazen",  # Add label to the table
             align = "c",
             col.names = c("Year", "Max Event", "Rank", "Fa Value", "Hazen Value"))  # Custom column names
...
knitr::kable(SugarGroveDischargeHazen2024, 
             ...
             digits = 2,
             label = "SugarGroveDischargeHazen",  # Table label
             align = "c",
             col.names = c("Year", "Max Event", "Rank", "Fa Value", "Hazen Value"))  # Custom column names

# Print Asheville discharge Hazen table with 2 decimal places
knitr::kable(AshevilleDischargeHazen2024, 
             caption = "Asheville Discharge Hazen Values for 2024", 
             digits = 2,
             label = "AshevilleDischargeHazen",  # Table label
             align = "c",
             col.names = c("Year", "Max Event", "Rank", "Fa Value", "Hazen Value"))  # Custom column names

# Print Pigeon River discharge Hazen table with 2 decimal places
knitr::kable(PigeonRiverDischargeHazen2024, 
             caption = "Pigeon River Discharge Hazen Values for 2024", 
             digits = 2,
             label = "PigeonRiverDischargeHazen",  # Table label
             align = "c",
             col.names = c("Year", "Max Event", "Rank", "Fa Value", "Hazen Value"))  # Custom column names
