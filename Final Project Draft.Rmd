...
MISSING (coming from below: This is our data! However, It needs to be cleaned. 
...
dataset_structure <- data.frame(
  ...
  Data_Type = c(
    "Categorical", "Numeric", "Numeric","Numeric", "Numeric (cubic meters/sec)", 
    "Numeric (feet)"
  )
)
...
```
All three locations show relatively low daily precipitation throughout the year, with notable peaks when Helene struck (around September). Looking at each graph individually, the Pigeon River location experienced the highest peak precipitation event during the hurricane, with over 6 inches in a single day, indicating it faced the most intense rainfall compared to the other two locations. However, this does not diminish the intensity observed in Asheville and Sugar Grove (5 inch peaks). Asheville's peaks appear slightly narrower and more intense compared to Sugar Grove's. Surprisingly, the graphs highlight Asheville's pattern of dry periods, which might explain why many people were surprised by the impacts of Helene in a place like Asheville, given its geographical location.
...
```
All three locations show relatively low discharge levels throughout most of the year, with significant spikes around September/october 2024, coinciding with the impact of Helene. This suggests that the extreme precipitation during Helene led to substantial water runoff and river discharge increases. Among the three, Asheville has the highest peak discharge which surpasses the other locations. Pigeon River shows a comparable degree of discharge, while Sugar Grove experiences the smallest peak among the three, although it is still notably high.

When analyzing the annual distribution, Sugar Grove and Pigeon River maintain relatively steady, low discharge levels throughout the year, while Asheville displays a more variable discharge pattern with multiple small peaks. This should give a sense to the heightened flood risk potential during extreme weather events like Helene. A key question that arises from these observations is whether these peaks are influenced by urbanization or the limited infiltration capacity of the city of Asheville which might also explain why Sugar Grove shows a smaller peak? 
...
```
These graphs clearly shows how Helene stands out as a historical event. All three locations (Sugar Grove, Asheville, and Pigeon River) experience occasional extreme rainfall events and relatively consistent patterns of lower daily precipitation over the decades. These historical records highlight that the extreme peaks observed in 2024, particularly during Helene are

**Discharge**
```{r, warning=FALSE}

CleanedDataSugarGrove$dateTime <- as.Date(CleanedDataSugarGrove$dateTime)
CleanedDataAsheville$dateTime <- as.Date(CleanedDataAsheville$dateTime)
CleanedDataPigeonRiver$dateTime <- as.Date(CleanedDataPigeonRiver$dateTime)
HeleneData_sg$dateTime <- as.Date(HeleneData_sg$dateTime)
HeleneData_ash$dateTime <- as.Date(HeleneData_ash$dateTime)
HeleneData_pg$dateTime <- as.Date(HeleneData_pg$dateTime)
...
SugarGroveDailyDischargeLinePlot <- ggplot(CleanedDataSugarGrove %>%
  ...
  geom_point(data = HeleneData_sg, aes(x = dateTime, y = `Mean Discharge`),
             ...
             size = 3) +
  scale_x_date(
    limits = as.Date(c("1954-01-01", "2024-12-31")),
    date_labels = "%Y"
  ) +
  labs(
...
```
All three locations show a generally stable pattern of daily mean discharge, marked by periodic spikes indicative of extreme events, with Helene standing out as a significant event in the last 70 years. Sugar Pine exhibits a noticeable peak that is unparalleled compared to the historical record, while Sugar Grove has experienced higher peaks of discharge in the past. However, it is important to highlight Asheville: despite appearing stable with its highest peak around 2005, the dataset is missing data for three critical days (September 27–29), precisely when Helene impacted the area. Therefore, the results for Asheville in this graph should be interpreted with caution.
