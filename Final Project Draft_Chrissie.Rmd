...
*Question 2: How these three counties experience rainfall and flooding impacts differently?*

*Question 3: How have these three counties historically experienced rainfall and flooding impacts?*


## Data Description

To analyze the impact and severity of Hurricane Helene, we utilized data from the United States Geological Survey (USGS). This data is invaluable for understanding extreme weather events due to its accuracy and daily collection. USGS stream gages, which measure daily discharge volumes in cubic feet per second, were chosen for their reliability.

Our analysis focused on daily precipitation trends, flood data, and hurricane patterns. To capture the diverse geographical and hydrological impacts of Helene across North Carolina, we selected three focal sites: Sugar Grove, Asheville, and Pigeon River. These locations were strategically chosen based on their proximity to Helene’s storm path, providing a comprehensive representation of the storm’s effects throughout Western North Carolina.
...
**Pigeon River**: Included because of its proximity to the Tennessee border, highlighting the need for cross-border storm management strategies.
...
To set up the analysis for the selected sites, retrieve data from the USGS using the "dataRetrieval" package. Begin by loading the necessary libraries, including dataRetrieval, dplyr, and tidyr and other relevant packages for analysis. We defined the site numbers and parameters of interest. The site numbers are: Sugar Grove (03479000), French Broad River in Asheville (03451500), and Pigeon River near Canton (03456500). We focused on the parameters stream discharge (00060), precipitation (00045), and gage height (00065). The statistical codes used are daily mean (00003) for discharge and gage height, and sum (00006) for precipitation.

Create a data retrieval function to streamline the process. This function should utilize readNWISdata from the dataRetrieval package to fetch data for the specified sites, parameters, and stat codes. The function should then clean the data by removing unnecessary "cd" columns, which indicate whether the data is official or provisional, as these columns do not significantly affect the analysis. Rename the remaining columns to include the parameter and statistical codes, ensuring clarity and consistency across datasets.

We applied the function to retrieve and clean data for each site: Sugar Grove, French Broad River, and Pigeon River. This step ensures uniformity in the data preparation process. After cleaning, we combined the datasets into a single, cohesive dataset, confirming that all columns are consistently labeled. This unified dataset allows for effective cross-site comparisons.
...
```{r, echo=FALSE, warning=FALSE, results='hide'}
#Then we decide we want to create an easy data retrieval function for my chosen parameters based on my previous retreival.
...
This plot illustrates the historical trend of daily precipitation (in inches) across Sugar Grove, Asheville, and Pigeon River. The highest peak, observed during Hurricane Helene, highlights the significant impact of this climate event on these counties.
...
The plot visualizes the historical mean daily gage height for three locations: Sugar Grove, Asheville, and Pigeon River. Asheville exhibits the most significant fluctuations, with peaks exceeding 15 feet, while Sugar Grove shows a more stable pattern. Pigeon River generally has lower gage heights and less pronounced peaks. Both Asheville and Pigeon River recorded their highest gage heights during Hurricane Helene.

*These graphs reaveal that Helene was responsible for some outlier behavior, but it doesn't account for seasonality and other trends*
...
# Suppress output of any calculations
invisible(any(is.na(CleanedDataAsheville$Discharge.7DayAvg)))
...
The Hazen number of 18 indicates that the maximum 7-day average precipitation  event in 2024 was an 18-year event, meaning it had about a 20% chance of  occurring in any given year (Hazen interval: 18 years, probability: 20%).
...
The table shows that for Sugar Grove in 2024, the maximum event had a Hazen value of 3.09. This indicates that the event had a return period of approximately 3.09 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 3.09 years. In other words, the event was not particularly unusual or extreme for Sugar Grove.
...
The table shows that for Asheville in 2024, the maximum event had a Hazen value of 2.43. This indicates that the event had a return period of approximately 2.43 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 2.43 years. In other words, the event was not particularly unusual or extreme for Asheville.
...
The table shows that for Pigeon River in 2024, the maximum event had a Hazen value of 47.33. This indicates that the event had a return period of approximately 47.33 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 47.33 years. In other words, the event was particularly unusual or extreme for Pigeon River compared to Asheville and Sugar Grove.
...
The table shows that for Sugar Grove in 2024, the maximum event had a Hazen value of 1.94. This indicates that the event had a return period of approximately 1.94 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 1.94 years. In other words, the event was not particularly unusual or extreme for Pigeon River.
...
The table shows that for Asheville in 2024, the maximum event had a Hazen value of 78. This indicates that the event had a return period of approximately 78 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 78 years. In other words, the event was particularly unusual or extreme for Asheville.
...
The table shows that for Pigeon River in 2024, the maximum event had a Hazen value of 20. This indicates that the event had a return period of approximately 20 years, meaning an event of this magnitude or greater is expected to occur, on average, once every 20 years. In other words, the event was particularly unusual or extreme for Pigeon River
...
Now that we have an easy function, we will download more sites in Western North Carolina that have precipitation, discharge, and gage height available. This will be a helpful visual in understanding the specific impacts in Sugar Grove, Asheville,and Pigeon River compared to the rest of the state. 
...
The analysis of Hurricane Helene in 2024 reveals it was a significant event across multiple parameters, particularly precipitation, discharge, and gage height. Comparing daily trends in precipitation and discharge across Sugar Grove, Asheville, and Pigeon River, Helene caused peak values of up to 6 inches of rainfall, 60,000 cfs of discharge, and 15 feet of gage height in Asheville. When we extended this analysis to historical data, we found that Helene's precipitation and gage height were exceptionally high, marking a 52-78 year event at various sites. In contrast, the discharge levels were less unusual for Sugar Grove and Asheville, with Hazen values indicating these events were expected every 2-3 years. This suggests that while Helene was significant, particularly in precipitation and gage height, the discharge levels were relatively typical for these regions.
